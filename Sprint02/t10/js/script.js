let _animal = new RegExp(/^[a-zA-Z]*$/);
let _gender = new RegExp(/male|female/i );
let _age = new RegExp(/^[0-9]*$/);
let i = false;
...
while (!(animal.length <= 20 && _animal.test(animal) && animal.length > 0));
//alert(animal);
...
while (!(_gender.test(gender) || gender.length === 0));
//alert(gender);
...
while (!(_age.test(age) && age.length <= 5 && age.length > 0));
//alert(age);
