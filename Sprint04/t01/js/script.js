...
let btn_fav = document.querySelector('#button-favorite');
renderOutput(0);

btn_all.addEventListener('click', switchAll);
btn_fav.addEventListener('click', switchFavorite);
...
function renderOutputFav(data_key) {
    //console.log(data);
    card_list.innerHTML = '';
    card_viev.innerHTML = '';
    let i = 0;
...
function switchFavorite() {
    if(btn_all.classList.contains('button_active')) {
        btn_all.classList.toggle('button_active');
        btn_fav.classList.toggle('button_active');
        show_fav = true;
        renderOutputFav(0);
